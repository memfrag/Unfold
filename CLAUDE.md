...
`FileNode` loads children lazily and shows only files the app can display (`isViewable` — Markdown *or* HTML) and directories with one *somewhere beneath them* (`containsViewableFile`, which checks each level's files before descending and skips symlinks so an ancestor link can't recurse forever). A Notion export is mostly per-page image folders; without that test the tree is dominated by rows that open nothing. The cost is that a genuinely empty directory has no row at all. `isMarkdown` stays a separate question from `isViewable`: it is the *editable* kind.

The tree keeps up with the folder by itself. **`FolderWatcher`** runs one FSEvents stream on the browsed root — not the per-file `DispatchSource` of `FileWatcher`, which would mean a descriptor per directory and no coverage of folders created after the window opened. It ignores *which* path changed: `FolderBrowserView.refreshTree()` reconciles the directories actually loaded, which is cheaper to reason about than mapping events back to nodes, and **View ▸ Refresh Folder** (Cmd+Shift+R, via `NavigationState.refreshTree`, nil in the single-document window so the item disables itself) runs exactly the same pass.

Two things that pass keeps right, and one it must not forget:
- `FileNode.reconcile` keeps the existing node wherever the URL is unchanged, so only real additions and removals move. Handing `OutlineGroup` a wholly new tree would take the disclosure state with it *and* discard the children those nodes had already loaded. The array is reassigned only when it genuinely differs.
- Collapsed directories stay lazy (`refresh()` returns early when `loadedChildren` is nil) and read fresh when opened.
- A page added inside a folder that was hidden for having nothing viewable beneath it has to make the **folder** appear — so the refresh reconsiders parents, not merely the directory that changed. Archive windows skip the watcher: an unpacked copy is private, and the selection is cleared if the displayed file is what disappeared.
...
The folder browser shows `.html`/`.htm` files too, through **`HTMLWebView`** — a second, much smaller `NSViewRepresentable`, not a mode of `MarkdownWebView`. An HTML file is already a page: WebKit loads it from disk with `loadFileURL(_:allowingReadAccessTo:)` and its stylesheets, scripts, images and sibling pages resolve relative to it. None of the Markdown machinery (shell, `_render`, TOC, editor, line sync) applies, so `FolderBrowserView` holds a `Displayed` enum — `.markdown(LooseFile)` versus `.html(URL)` — rather than pretending one pipeline serves both. Read access is granted for the whole browsed root, not the file's own directory, so a page pulling CSS from further up still renders.

Navigation is the app's, not WebKit's. A page left to follow its own links would leave the sidebar on the old file and the window's history unaware, so `decidePolicyFor` cancels **link activations in the main frame** and hands them to `NavigationState.openFile` — the same door Markdown links use, so one history and one selection cover both kinds of file. Three carve-outs: the initial load, redirects and iframes are allowed through (otherwise the page can't load at all); a `#fragment` jump within the same document is allowed so in-page anchors work natively (and is therefore *not* a history entry, unlike Markdown's, which are); and a non-viewable target goes to `NSWorkspace`. Scroll offsets are reported by an injected script into the same `currentScrollY`/`pendingScroll` machinery the Markdown side uses. Reload works through `NavigationState.reloadDisplay`, a hook the pane installs, since there is no coordinator bridge to render through.

**`URL.physicalURL`** (`URL+PhysicalPath.swift`) exists because of a trap worth remembering: `resolvingSymlinksInPath()` and `standardizedFileURL` both map `/private/tmp` back to `/tmp` — the opposite of resolving. `FileManager` hands out physical paths, so a folder opened through a symlink produces two spellings of the same place, and both WebKit's file sandbox ("Ignoring request to load this main resource because it is outside the sandbox" — a blank page, nothing on screen) and the folder browser's is-this-link-inside-the-root test compare them as strings. Canonicalize both sides with `realpath` before comparing.
...
- Edits to the unpacked copy would never reach the archive, so those windows are **read-only**: `FolderBrowserView` clears `NavigationState.isEditable` for every file it shows, which folds into `canEdit` (covering the toolbar button, Cmd+Shift+E *and* the menu item, which all route through it), and drops the mutating context-menu items. Note the two are different questions: the view's own `isReadOnly` is the archive (it governs the context menu, which stays available for a normal folder), while `isEditable` is about the file on screen — an HTML page clears it too.
